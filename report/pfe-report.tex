...
% Strong override: force LaTeX plain pages to behave like fancy pages
\makeatletter
...
%% TITLE PAGE
%%─────────────────────────────────────────────────────────────
\includepdf[pages=2]{frontmatter/frontback.pdf}
%%─────────────────────────────────────────────────────────────
%% DEDICATION
%%─────────────────────────────────────────────────────────────
...
All nodes run Ubuntu Server 24.04.4 LTS as KVM virtual machines on a Proxmox 8.x hypervisor, connected via a dedicated \texttt{172.16.10.0/24} VLAN.
...
\toprule
\textbf{VM Name} & \textbf{IP} & \textbf{Role} & \textbf{OS} & \textbf{Workloads} \\
\midrule
...
\subsection{Kubernetes Namespace Organisation}
...
\begin{tabular}{lllll}
\toprule
\textbf{Namespace} & \textbf{Pods} & \textbf{Workload Type} & \textbf{Storage} & \textbf{Uptime} \\
\midrule
\texttt{wazuh}   & 4 & StatefulSet + Deployment & PVC local-path & 75+ days \\
\texttt{thehive} & 1 & Deployment               & PVC 5 Gi       & 75+ days \\
\texttt{nids}    & 2 & DaemonSet                & HostPath       & 35+ days \\
\bottomrule
\end{tabular}
...
\begin{itemize}
  \item \textbf{SOC Analyst:} Monitors alerts in Wazuh Dashboard, manages cases in TheHive, triggers Cortex analysers, reviews Shuffle workflow logs
  \item \textbf{Attacker (threat actor):} Performs brute force, port scans, and endpoint attacks---triggering the detection chain
  \item \textbf{Wazuh Agent (system actor):} Collects logs, executes active responses, reports system state every second
  \item \textbf{DevOps Engineer:} Pushes configuration changes to GitHub, triggering the CI/CD and AWX pipeline
\end{itemize}

\subsection{Sequence Diagram --- SSH Brute Force Detection Chain}

The complete automated response chain for a detected SSH brute-force attack proceeds as follows:

\begin{enumerate}
  \item Attacker sends 30 failed SSH logins within 120 seconds to \texttt{ubunttest} (172.16.10.11). \quad \textbf{T0}
  \item Target's \texttt{/var/log/auth.log} records \texttt{Invalid user} events (rule 5710, level 5).
  \item Wazuh agent reads auth.log and forwards 5 consecutive 5710 matches to Manager Worker.
  \item Manager Worker's rule engine matches 5$\times$ rule 5710 in 120 s $\Rightarrow$ rule \textbf{100100} fires (level 12). \quad \textbf{T1 = T0+2.17 s}
  \item Manager dispatches Active Response \texttt{firewall-drop600} to the agent. \quad \textbf{T2 $\approx$ T1+1 s}
  \item Agent executes \texttt{iptables -I INPUT -s \$ATTACKER -j DROP}, blocking the attacker. \quad \textbf{$<$ 4 s from T0}
  \item \texttt{wazuh-integratord} sends webhook to Shuffle SOAR; \texttt{custom-thehive.py} creates a TheHive case.
\end{enumerate}
...
\end{itemize}
\clearpage
\includepdf[pages=1]{frontmatter/frontback.pdf}
\end{document}
